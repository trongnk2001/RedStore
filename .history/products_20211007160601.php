    ...
    <div class="small-container">
        <div class="row row-2">
            ...
            <select name="" id="" onchange="this.options[this.selectedIndex].value && (window.location = this.options[this.selectedIndex].value);">
                <option value="?field=price&sort=normal">Sắp xếp theo</option>
                <option value="?field=price&sort=desc">Giá (cao - thấp)</option>
                <option value="?field=price&sort=asc">Giá (thấp - cao)</option>
                <option value="">Bán chạy</option>
                <option value="?field=price&sort=new">Mới nhất</option>
            </select>
        </div>
        ...
        <div class="row">
            ...
            if($orderField == true && $orderSort == "desc"){
                $sql_query= "SELECT * FROM `product` ORDER BY price DESC";
            }
            elseif($orderField == true && $orderSort == "asc"){
                $sql_query= "SELECT * FROM `product` ORDER BY price ASC";
            }
            elseif($orderField == true && $orderSort == "normal"){
                $sql_query= "SELECT * FROM `product` ORDER BY productID ASC";
            }
            elseif($orderField == true && $orderSort == "new"){
                $sql_query= "SELECT * FROM `product` ORDER BY productID DESC";
            }
