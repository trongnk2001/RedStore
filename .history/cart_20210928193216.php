<?php
    ...
    if(isset($_POST['themgiohang'])){
        ...
        if($row){
            $new_Product = array(array('name' => $row['name'], 'id'=>$id, 'soluong'=>$sl, 'price'=>$row['price'], 
            'img'=>$row['img']));
            ...
            if(isset($_SESSION['cart'])){
                ...
                foreach($_SESSION['cart'] as $cart_item){
                    if($cart_item['id']==$id){
                        $product[] = array('tensanpham' => $cart_item['name'], 'id'=>$cart_item['id'], 'soluong' =>$soluong = $soluong + $sl, 'giasp'=>$cart_item['price'], 
                        'img'=>$cart_item['img']);
                        $found = true;
                    }
                    else{
                        $product[] = array('tensanpham' => $cart_item['name'], 'id'=>$cart_item['id'], 'soluong'=>$sl, 'giasp'=>$cart_item['price'], 
                        'img'=>$cart_item['img']);
                    }
                }
                if($found == false){
                        ...
                        while ( $row = mysqli_fetch_array($query) ){
                            ...
                            ?>
                                <tr>
                                    <td>
                                        <div class="cart-info">
                                            <?php echo ' <img src="'.$rerult.'" alt="" ';?>
                                            <div>
                                                <p><?php echo $row[2] ?></p>
                                                <small><?php echo 'Giá : '.$row[10] ?></small>
                                                <br>
                                                <a href="">Remove</a>
                                            </div>
                                        </div>
                                    </td>
                                    <td><?php '<input type="number" value="$_POST["sl"]">' ?></td>
                                    <td>499.000đ</td>
                                </tr>
