<?php
    ...
    if(isset($_POST['themgiohang'])){
        ...
        $sl=$_POST["sl"];
        $soluong = 0 ;
        $sql_query = "SELECT * FROM product WHERE productID='".$id."' LIMIT 1";
        $query = mysqli_query($links,$sql);
        $row = mysqli_fetch_array($query);
        if($row){
            $new_Product = array(array('tensanpham' => $row[2], 'id'=>$id, 'soluong'=>$soluong+1, 'giasp'=>$row[10], 
            'img'=>$row[3]));
            ...
            if(isset($_SESSION['cart'])){
                ...
                foreach($_SESSION['cart'] as $cart_item){
                    if($cart_item['id']==$id){
                        $product[] = array('tensanpham' => $cart_item['name'], 'id'=>$cart_item['id'], 'soluong'=>$cart_item['soluong']+$sl, 'giasp'=>$cart_item['price'], 
                        'img'=>$cart_item['img']);
                        $found = true;
                    }
                    else{
                        $product[] = array('tensanpham' => $cart_item['name'], 'id'=>$cart_item['id'], 'soluong'=>$cart_item['soluong']+1, 'giasp'=>$cart_item['price'], 
                        'img'=>$cart_item['img']);
                    }
                }
            }
                ...
                <tr>
                    <td>
                        <div class="cart-info">
                            ...
                            <div>
                                <p>Áo PUMA Active Big Logo Tee</p>
                                <small>Giá : 499.000đ</small>
                                <br>
                                <a href="">Remove</a>
                            </div>
                        </div>
                ...
                <tr>
                    <td>
                        <div class="cart-info">
                            ...
                            <div>
                                <p>Giày Nam đen Rogue 2</p>
                                <small>Giá : 795.000đ</small>
                                <br>
                                <a href="">Remove</a>
                            </div>
                        </div>
