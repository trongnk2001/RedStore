                         ...
                         case "submit":
                            ...
                            elseif (isset($_POST['update_checkout'])){
                                ...
                                if ($error == false && !empty($_POST['soluong'])) {
                                    ...
                                    $idcus = $add[0][0];
                                    var_dump('IDDDDDD:'.$idcus);
                                    $last_id = mysqli_insert_id($links);
                                    $mysqltime = date ("Y-m-d H:i:s", $phptime);
                                    $insertOrder = mysqli_query($links, "INSERT INTO orderinf (orderID, customerID, total, orderDate, status) VALUES ('', '$idcus', '$tongtien', '$mysqltime', '1')");
                                        
                                    for($i = 1; $i <= count($array_cart[0]); $i++){
                                        echo '<pre>';
                                        var_dump($array_cart[0][$i-1]['id']);
                                        echo '</pre>';
                                    ...
                                    $insertOrderDetails = mysqli_query($links, "INSERT INTO `orderdetails` (`id`, `orderID`, `productID`, `price`, `quantity`, `total`) VALUES ('', '$idord','$products', '$tongtien', '$mysqltime', '1')");
                            }
