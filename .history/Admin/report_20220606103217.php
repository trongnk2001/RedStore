...
<body class="nav-md">
    <div class="container body">
        ...
        <?php 
            ...
            $date = "SELECT YEAR(orderDate) AS year, MONTH(orderDate) AS month FROM orderinf GROUP BY year,month";
            $result1 = mysqli_query($links, $date);
            $daterow = mysqli_fetch_row($result_kh);

            $month = $daterow['month'];
            $year = $daterow['year'];

            $chart_data = '';
            while($row = mysqli_fetch_array($daterow))
            {
            $chart_data .= "{ year:'".$year."', month:".$month."}, ";
            }
